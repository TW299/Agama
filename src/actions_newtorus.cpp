...
namespace actions {
		...
		*/
		...
		public:
			torusFitter(const Actions& _J,
				...
				const PtrToyMap _ptrTM, GenFncFit& _GFF) :
			    J(_J), pot(_pot), freqScale(1000 * _freqScale), GFF(_GFF), ptrTM(_ptrTM)
			{
				N1 = GFF.numParams();
			}
			virtual unsigned int numVars() const {
		...
		// derivatives w.r.t. coordinates/momenta
				...
				if (dHdParams) {
					...
					for (unsigned int p = 0; p < GFF.numParams(); p++) {
						...
						Actions dbyP = GFF.deriv(indPoint, p, &params[0]);
			...
			double computeHamiltonianDisp(const std::vector<double>& params, double& Hbar)
			{
				...
				for (int indPoint = 0; indPoint < GFF.numPoints(); indPoint++) {
				...
				NANfrac = (double)Nnan[0] / (double)GFF.numPoints();
				NANbar += NANfrac;
			...
			/* Compare analytic and numerical derivatives
			 * of H -<H> wrt to the Sk at the grid of points on
			 * the torus
			 */
			void testit(std::vector<double> params, int N) {
				int nPts = GFF.numPoints(), nPars = GFF.numParams();
	...
	Torus InterpTorus(const double x, const Torus& T0, const Torus& T1){
		if (x == 1) return T0;
		if (x == 0) return T1;
		const double xp = 1 - x;
	...
	Torus TorusGenerator::interpTorus(const double x, const Torus& T0, const Torus& T1) const{
		Torus T0n=T0, T1n=T1;
	...
	Torus TorusGenerator::interpTorus(const double x, const std::vector<double>& xs,
	...
	PerturbingHamiltonian interpPerturbingHamiltonian(double x,
	...
	eTorus TorusGenerator::interpeTorus(const double x, const eTorus& eT0, const eTorus& eT1) const{
		if (x == 1) return eT0;
		if (x == 0) return eT1;
		return eTorus(interpTorus(x, Torus(eT0), Torus(eT1)),
			interpPerturbingHamiltonian(x, eT0.pH, eT1.pH));
	}
	eTorus TorusGenerator::interpeTorus(const double x, const std::vector<double>& xs,
	...
	*/
	...
	void Torus::read(FILE* ifile){
		double Delta, Js, b;
		fscanf(ifile, "%lf %lf %lf %lf %lf %lf %lf %lf %lf %lf\n",
			&J.Jr, &J.Jz, &J.Jphi, &freqs.Omegar, &freqs.Omegaz, &freqs.Omegaphi,
			&E, &Delta, &Js, &b);
		PTIso newPT(Delta);
	...
	std::vector<std::complex<double> > PerturbingHamiltonian::get_hn(const GenFncIndex& I,
		...
		for (int i = 0; i < indices.size(); i++) {
			...
			if (Rats[0] >= 1) multiples.push_back((float)Rats[0]);
			else multiples.push_back(1 / (float)Rats[0]);
		}
		return Hs;
	}
...
#pragma omp parallel for schedule(dynamic) 
	...
	void writeClosed(std::vector<coord::PosVelCyl>& shell, Actions J, PtrToyMap& TM, PtrToyMap& baldTM) {
		FILE* ofile=fopen("writeClosed.dat", "w");
		int npt = 200;
		fprintf(ofile, "%d\n", npt);
	...
	PtrToyMap TorusGenerator::chooseTM(GenFncFit& GFF0,
	...
	Torus TorusGenerator::giveBaseTorus(const Actions& J, const PtrToyMap& ptrTM) const {
		...
		//GenFncFit GFF(indices, 8, 6, J);
		GenFncFit GFF(indices, 1, 1, J);
		torusFitter TF(J, pot, 1, ptrTM, GFF);
		Frequencies freqs;
		GenFncDerivs dPdJ;
		bool negJr, negJz;
	...
	Torus TorusGenerator::fitTorus(const Actions& J, const double tighten,
		...
		converged = (HdispTM < tol * freqScale * Jscale);		
		do {
			double Hdisp_old = Hdisp;
			unsigned int numTerms_old = GFF0.numTerms();
			...
			if(Loop>8)
				stuck = fabs(1 - rep[Loop] / rep[Loop - 1]) < 1e-2 && GFF.numTerms() > numTerms_old;
			if (converged || (Loop > 1 && stuck)) break;
			if (GFF.numTerms() > numTerms_old && Hdisp > Hdisp_old) {//something wrong: back up 
				indices = GFF0.indices;
...
//		for(int i=0;i<Loop;i++) printf("%g ",rep[i]); printf("\n");
	...
	Torus TorusGenerator::fitTorusNoFourier(const Actions& J, const double tighten,
		...
		double Jscale, freqScale, Rsh, HdispTM;
		PtrToyMap ptrTM(chooseTM(GFF0, params, J, Jscale, freqScale, Rsh,
					 HdispTM, ToyMapType, logfile, 0, 0));
		double tolerance = 1e-9;//controls optimisation of the given Sn
		double tol = defaultTol * tighten;
		double Hbar, Hdisp = 1e20, Htarget = tol * freqScale * Jscale;
		bool converged = false;
	...
	Torus TorusGenerator::fitBaseTorus(const Actions& J, const double tighten, const ToyPotType ToyMapType) const {
		...
		GenFncIndices indices = makeGridIndices(nrmax, nzmax);
		std::vector<double> params(indices.size(), 0);
//		GenFncFit GFF0(indices, 8, 6, J);
		GenFncFit GFF0(indices, 1, 1, J);
		double Jscale, freqScale, Rsh, HdispTM;
		PtrToyMap ptrTM(chooseTM(GFF0, params, J, Jscale, freqScale, Rsh,
					 HdispTM, ToyMapType));
		if(logfile) fclose(logfile);
	...
	Torus TorusGenerator::fitFrom(const Actions& J, const Torus& T, const double tighten) const {
		...
		GenFncDerivs dPdJ;
		bool negJr, negJz;
		GenFncFit GFFP(indices, 2*timesr, 2*timesz, J);
		torusFitter TFS(J, pot, freqScale, ptrTM, GFFP);
...
//		return interpR.value(L, (J.Jr + J.Jz) / L);
	...
	ActionAngles ActionFinderTG::actionAngles(const coord::PosVelCyl& point, Frequencies* freq) const {
		Torus T;
		ActionAngles aa(actionAnglesTorus(point, T));
		if (freq) *freq = T.freqs;
		return aa;
	}
	ActionAngles ActionFinderTG::actionAnglesTorus(
