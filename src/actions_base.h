...
struct Angles {
    ...
    Angles(Frequencies freq) : thetar(freq.Omegar), thetaz(freq.Omegaz), thetaphi(freq.Omegaphi) {}
...
typedef struct DerivAct<coord::Cyl> DerivActCyl;
...
typedef struct DerivAng<coord::Cyl> DerivAngCyl;
...
class BasePointTransform{
    ...
    /** maps in both given coordinate system (either cylindrical or spherical)
     ** and other coordinate system. In other coordinate system there is the
     ** usual transformation from spherical or cylindrical to given
     ** coordinate system before mapping*/
    ...
    /* Provide also derivates of transformed phase space point w.r.t original
     *  point. Note derivative is always in given coordinate system*/
    ...
    // fncs to show maping generated by Fourier series
		virtual double T2v(const double) const=0;
...
class BaseToyMap{
	public:
		...
		virtual const char* name() const=0;
		// Gets parameters of Toy potential.
