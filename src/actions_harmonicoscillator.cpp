...
namespace actions {
	double HarmonicOscillator::H(coord::PosMomCyl& rp) const {
		double H = .5 * (pow_2(rp.pR) + pow_2(rp.pz))
			+ .5 * (pow_2(omegaR * rp.R) + pow_2(omegaz * rp.z));
		return rp.pphi == 0 ? H : H + .5 * pow_2(rp.pphi / (rp.R));
	}
	Actions HarmonicOscillator::pq2J(const coord::PosMomCyl& pc) const {
	...
	coord::PosMomCyl HarmonicOscillator::aa2pq(const ActionAngles& aa, Frequencies* freqs,
	...
	coord::PosMomCar HarmonicOscillator::aa2pqCar(const ActionAngles& aa, Frequencies* freqs,
	...
	ActionAngles HarmonicOscillator::pq2aa(const coord::PosMomCyl& pc, Frequencies* freqs) const {
		double Jz = .5 *  (pow_2(pc.pz) + pow_2(omegaz * pc.z))/omegaz;
		double HR = .5 * (pow_2(pc.pR))
			+ .5 * (pow_2(omegaR * pc.R));
		if (pc.pphi != 0) HR += .5 * pow_2(pc.pphi / (pc.R));
	...
	coord::PosMomCyl HarmonicOscillator::aa2pq(const ActionAngles& aa,
	...
	coord::PosMomCar HarmonicOscillator::aa2pq(const ActionAngles& aa,
	...
	coord::PosMomCar HarmonicOscillator::aa2pqCar(const ActionAngles& aa,
		coord::PosMomCar& drdomegar, coord::PosMomCar& drdomegaz) const {
		double z = sqrt(2 * aa.Jz / omegaz) * sin(aa.thetaz);
		double pz = sqrt(2 * aa.Jz * omegaz) * cos(aa.thetaz);
		double x = sqrt(4 * aa.Jr / omegaR) * sin(aa.thetar / 2);
	...
	HarmonicOscillator interpHarmonicOscillator(const double x, const HarmonicOscillator& os0, const HarmonicOscillator& os1) {
		const double xp = 1 - x;
		return HarmonicOscillator(os0.omegaR * x + os1.omegaR * xp, os0.omegaz * x + os1.omegaR * xp);
	}
}//namespace actions