...
double I3val(const potential::OblatePerfectEllipsoid& pot, const double E,
    ...
    Phi=((potential::BasePotential&)pot).value(coord::PosCyl(R,0,0));
...
class RFinder: public math::IFunctionNoDeriv {
	...
	public:
		...
		{
            double Phi=((potential::BasePotential&)pot).value(coord::PosCyl(R,0,0));
...
int main(){
    ...
    double Phi, I3zero = I3val(*ptrPEpot,E,Rpmax-1e-10,Lz,Phi);//for zero-v curve
    actions::ActionFinderAxisymStaeckel af(ptrPEpot);
	for(int j=0; j<nc; j++){
		double Rmin=1.1*Rpmin, Rmax=.95*Rpmax, R=Rmax;
		double I3 = I3val(*ptrPEpot, E, R, Lz, Phi);
		RFinder RF(*ptrPEpot, E, I3, Lz);
		...
		getCurve(*ptrPEpot,np,E,Lz,I3,Rin,Rout,Rs,vRs);
		{
			bool onlyTM=false;//true;//
			double vR=0, vphi=Lz/R, vz=sqrt(2*(E-Phi)-pow_2(vphi)); 
			coord::PosVelCyl Rzp(R,0,0,vR,vz,vphi);
			actions::Frequencies omegas;
			actions::Actions J;actions::Angles th;
            af.eval(Rzp,&J,&th,&omegas);
            actions::ActionAngles aa(J,th);
